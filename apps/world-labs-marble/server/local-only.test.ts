...
/** Node fetch strips Origin/Host; send them with http.request. */
function rawRequest(
  url: string,
  init: {
    method?: string
...
function isLoopbackHex(address: string) {
  const value = address.toLowerCase()
  return (
    value === "0100007f" ||
    value === "00000000000000000000000001000000" ||
    value === "0000000000000000ffff00000100007f"
  )
}
...
function expectLoopbackListener(port: number) {
  ...
  assert.ok(listeners.length > 0, `expected a listener on port ${port}`)
  for (const fields of listeners) {
    const ip = fields[1].split(":")[0]
    assert.ok(
      isLoopbackHex(ip),
      `port ${port} bound to ${ip}, expected loopback`,
    )
  }
}
...
describe("local-only playground", { concurrency: 1 }, () => {
  test("API rejects foreign origins and hosts before generating and binds only to loopback", async () => {
    ...
    await withApp(`PORT=${port}\n`, undefined, async (start) => {
      ...
      for (const headers of [
        ...
        const response = await rawRequest(`${base}/api/generate`, {
          method: "POST",
          headers,
      ...
      const preflight = await rawRequest(`${base}/api/generate`, {
        method: "OPTIONS",
        headers: {
      ...
      assert.equal((response.json() as { source: string }).source, "fixture")
    ...
    test(`Vite proxies to the API with ${scenario} PORT and stays on loopback`, async () => {
      ...
      await withApp(
        ...
        async (start) => {
          ...
          const response = await rawRequest("http://127.0.0.1:5173/api/generate", {
            method: "POST",
            headers: {
          ...
          assert.equal(response.status, 200)
          const operation = response.json() as { source: string; operation_id: string }
          ...
          const rejected = await rawRequest("http://127.0.0.1:5173/api/generate", {
            method: "POST",
            headers: {
    ...
    try {
      ...
      await withApp(
        ...
        async (start) => {
          ...
          const blocked = await rawRequest(`${base}/api/generate`, {
            method: "POST",
            headers: {
              Origin: "https://attacker.example",
              "Content-Type": "application/json",
          ...
          const allowed = await rawRequest(`${base}/api/generate`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ text_prompt: "local mock only" }),
          })
          assert.equal(allowed.status, 200)
          assert.equal((allowed.json() as { source: string }).source, "live")
